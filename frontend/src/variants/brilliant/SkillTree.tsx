...
function MasteryRing({ mastery, size = 40 }: { mastery: number; size?: number }) {
    ...
    const color =
        ...
        mastery >= 40 ? "#26C6DA" :   // teal-400
        mastery > 0   ? "rgba(38,198,218,0.3)" : // teal-400/30
                        "rgba(38,198,218,0.15)";  // teal-400/15
...
export function SkillTree({
        ...
        return (
            <div className="h-full bg-[#0F0F0F] flex items-center justify-center">
                <div className="text-center space-y-4">
                    ...
                    <p className="text-base text-teal-200">{loadingMessage || "Building skill tree..."}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="h-full flex flex-col bg-[#0F0F0F] relative">
            ...
            <div className="shrink-0 px-5 pt-5 pb-3">
                    ...
                    <div className="flex items-center gap-2">
                        {resourceCount > 0 && (
                            <button
                                ...
                                className="p-2 rounded-xl hover:bg-white/10 text-teal-200 transition-colors relative"
                            >
                                <FileText className="w-5 h-5" />
                                <span className="absolute -top-0.5 -right-0.5 w-5 h-5 rounded-full bg-teal-500 text-white text-[10px] font-bold flex items-center justify-center">
                                    {resourceCount}
                                </span>
                            </button>
                        )}
                        <button
                            ...
                            className={cn(
                                ...
                                isUploading
                                    ...
                                    : "text-teal-200 border-teal-400/20 bg-teal-500/10 hover:bg-teal-500/20",
                            )}
                        >
                            {isUploading ? (
                ...
                <div className="space-y-1.5">
                    ...
                    <p className="text-base text-teal-100">
                        ...
                        {totalPresence > 0 && (
                            <span className="ml-2">&middot; {totalPresence} learners active</span>
                        )}
                    </p>
                </div>
            ...
            {resourceCount === 0 && !hintDismissed && (
                <div className="mx-5 mb-3">
                    <div className="rounded-xl bg-teal-500/8 border border-teal-500/15 px-4 py-3 flex items-start gap-3">
                        <Sparkles className="w-4 h-4 text-teal-300 mt-0.5 shrink-0" />
            ...
            {/* Regen banner */}
            {showRegenBanner && (
                <div className="mx-5 mb-3 rounded-xl bg-teal-500/15 border border-teal-400/30 px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                            ...
                            className="flex items-center gap-1.5 text-sm font-medium text-white bg-teal-600 hover:bg-teal-500 px-4 py-2 rounded-lg transition-colors disabled:opacity-50"
                        >
                ...
                {syllabus.units.map((unit) => (
                    <div key={unit.id} className="mb-6">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-base font-semibold text-teal-100">{unit.name}</span>
                        </div>

                        {/* Topics list */}
                        <div className="ml-3 border-l-2 border-teal-400/20 pl-4 space-y-2">
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            "bg-neutral-900/40 border",
                                            "hover:bg-neutral-900/60 hover:border-teal-400/25",
                                            isRecommended
                                                ? "border-teal-400/40"
                                                : "border-teal-400/15",
                                        )}
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                {isRecommended && (
                                                    <span className="shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded bg-teal-500/20 border border-teal-400/30 text-teal-200">
                                                        NEXT
                                                    </span>
                                                )}
                                            </div>
                                            <div className="flex items-center gap-2 mt-0.5">
                                                ...
                                                {p > 0 && (
                                                    <span className="flex items-center gap-0.5 text-xs text-teal-300/50">
                                        ...
                                        <Play className="w-4 h-4 text-teal-400/60 shrink-0" />
                                    </button>
                                );
                            })}
                        </div>
            ...
            >
                <div className="space-y-3">
                    {subjectResources.length === 0 ? (
                        <p className="text-sm text-teal-200/60 text-center py-4">
                        ...
                        subjectResources.map((resource) => (
                            <div
                                key={resource.id}
                                className="flex items-center justify-between bg-[#1A1A1A] border border-teal-400/20 rounded-xl px-4 py-3"
                            >
                                <div className="min-w-0">
                                    <div className="text-sm text-white truncate">{resource.file_name}</div>
                                    <div className="text-xs text-teal-200/50 mt-0.5">
                    ...
                    {subjectResources.length > 0 && onGenerateFromResources && (
                        <div className="pt-2 border-t border-teal-400/10">
                            {generationProgress && generationProgress.step !== "complete" && generationProgress.step !== "error" ? (
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-teal-600/80 hover:bg-teal-600 text-white text-sm font-medium transition-colors disabled:opacity-50"
                                >
