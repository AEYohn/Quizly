...
"""
    ...
    def _generate_mock(
        ...
        # LLM is required for proper exit ticket generation
        concept = weakest.get("concept", "unknown")
        accuracy = weakest.get("accuracy", 0.0)
        
        return {
            "student_id": student_id,
            "target_concept": concept,
            "micro_lesson": f"[LLM Required] Micro-lesson for {concept}",
            "question": {
                "prompt": f"[LLM Required] Follow-up question about {concept}",
                "options": [
